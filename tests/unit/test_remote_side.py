...
def test_starts_are_deterministic_per_index_and_match_across_sides():
    ...
    for i in range(1, 7):
        a, b = us._start_positions(i), us._start_positions(i)
        assert a == b                                   # stable across reruns
        assert us._start_positions(i) == them._start_positions(i)  # both teams agree
        cop, thief = a
...
def test_result_reads_winner_and_scores_from_status():
    us = _side("ahk-yosi")
    r = us._result(2, {"status": "thief_win", "thief_moves": 25})
